...
export function SelectionOptionA4aCreate() {
        
        const router = useRouter();

        const [err, setErr] = useState(false);

        const SelectionOptionA4aCreation = async (e) => {
            e.preventDefault();
            ...
            try {
                ...
                const res = await fetch("/api/selections/familyandfriends/selectionoptiona/infosection4a/create", {
                    method: "POST",
                    headers: {
    ...
    return (
        <>
            <form onSubmit={SelectionOptionA4aCreation}
                  className='row w-95'  
                >
                <ConsumerEmail />
                <InfoSectionAName />
